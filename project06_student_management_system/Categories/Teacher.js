...
import { Teacher } from '../Classes/Classes.js';
let sleep = () => new Promise((r) => setTimeout(r, 1000));
// Add Teacher Function
export async function AddTeacher(DetailsInputs, teachers) {
    const teacher_name = await DetailsInputs('', 'Name');
    const teacher_age = await DetailsInputs('number', 'Age');
...
export async function ViewTeachers(teachers, courses) {
    if (!teachers.length) {
    ...
    if (index <= teachers.length - 1 && index >= 0) {
        ...
        let course_id = await input['course'];
