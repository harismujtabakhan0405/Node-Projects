...
export async function AddStudent(DetailsInputs, students) {
    const name = await DetailsInputs('', 'Name');
    const age = await DetailsInputs('number', 'Age');
...
export async function ViewStudents(students, courses) {
    if (!students.length) {
