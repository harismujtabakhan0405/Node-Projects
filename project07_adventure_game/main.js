...
console.log(chalk.bold.blue(`   <<<================================>>>`));
console.log(chalk.bold.blue(`<<<==========>>>  ${chalk.redBright.bold.underline('ADVENTURE GAME')}  <<<==========>>>`));
console.log(chalk.bold.blue(`   <<<================================>>>\n`));
console.log(chalk.bgRgb(112, 79, 1).black(' INSTRUCTIONS :'));
...
let attackAnimation = (enemy) => new Promise((r) => {
    ...
    const animation = setInterval(() => {
        ...
        process.stdout.write(chalk.bgBlack.whiteBright(`___________${chalk.bgRgb(0, 55, 0).whiteBright('You =>')}${stepsLeft.join('')}${chalk.bgRgb(55, 0, 0).whiteBright(`<= ${enemy}`)}${stepsDone.join('')}__________`));
        if (!stepsLeft.length) {
