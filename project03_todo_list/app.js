...
console.log(chalk.bold.rgb(204, 204, 204)(`<<<==========>>>  ${chalk.redBright.bold.underline('TODO APP')}  <<<==========>>>`));
console.log(chalk.bold.rgb(204, 204, 204)(`   <<<================================>>>\n`));
let todos = [{ text: 'dummy', id: Math.floor(Math.random() * 9999999999), completed: false }];
...
const Options = async () => {
    ...
    let value = await input['\x1B[38;2;255;255;160mWhat You Want To Do\x1B[39m'];
    return value;
};
const AddTodo = async () => {
    let value;
...
const RemoveTodo = async () => {
    ...
    let value = await input['\x1B[38;2;255;255;160mDelete TODO\x1B[39m'];
...
const DisplaySingleTodo = async (todo) => {
