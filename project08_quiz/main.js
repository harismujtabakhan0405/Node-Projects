...
console.log(chalk.bold.blue(`   <<<================================>>>`));
console.log(chalk.bold.blue(`<<<==========>>>  ${chalk.redBright.bold.underline('QUIZ')}  <<<==========>>>`));
console.log(chalk.bold.blue(`   <<<================================>>>\n`));
...
async function startQuiz(questions) {
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        if (questions[i].code) {
            ...
            let set_code = questions[i].code?.replace(/_/g, `   `).split('@');
...
async function Result(name, answers, questions) {
    ...
    let percentage = (points * 100) / questions.length;
...
while (true) {
    ...
    let value = await input['\x1B[38;2;255;255;160mDo You Want To Reattempt Quiz?\x1B[39m'];
    if (!value) {
